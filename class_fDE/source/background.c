...
int background_w_fld(
  ...
  case faDE2:
    *integral_fld = 1. + pba->fa_fld*(1-a)+ pba->dfa_fld*pow((1-a),2);
    break;
  case fpDE2:
    *integral_fld = 1+(1-a)*(pba->dfp_fld + 2*(pba->fp_fld-1)/(1-pba->ap_fld)) -pow((1-a),2)/(1-pba->ap_fld)*(pba->dfp_fld + (pba->fp_fld-1)/(1-pba->ap_fld));
    break;
  case EDE:
...
int background_derivs(
  ...
  if (pba->has_fld == _TRUE_) {
	if (pba->fluid_equation_of_state == fpDE) {  
		...
		dy[pba->index_bi_rho_fld] = (2*a*((1-pba->ap_fld)*pba->dfp_fld-pba->fp_fld+1)-pba->ap_fld*(pba->ap_fld*pba->dfp_fld-2*pba->fp_fld+2)+pba->fp_fld)/(pow((1-pba->ap_fld),2))*pba->Omega0_fld * pow(pba->H0,2)*a;
	}
	else if (pba->fluid_equation_of_state == faDE2) {  
		dy[pba->index_bi_rho_fld] = -(pba->fa_fld+pba->dfa_fld*(1-a))*pba->Omega0_fld * pow(pba->H0,2)*a;
	}
	else {
